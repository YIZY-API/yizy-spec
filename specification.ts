import type { ProgrammingLanguage } from "./constants.ts";

/**
 * A module providing functions and interfaces to create an YIZY API Spec
 * @module
 */
...
export interface Endpoint {
  ...
  name: string | NameMap;
  description: string;
  requestModel: ObjectType | null;
  responseModel: ObjectType | null;
}
...
export function nullableObjectType(
  name: string | NameMap,
  fields: Field[],
): NullableObjectType {
  return {
...
export function isObjectType(type: DataType): boolean {
...
export function isNullableArrayType(type: DataType): boolean {
  if (typeof type === "string") return false;
  const t: NonPrimitiveType = type as NonPrimitiveType;
  return t.type == TypeIdentifier.NullableArrayType;
}

export function isRefType(type: DataType): boolean {
  if (typeof type === "string") return false;
  const t: NonPrimitiveType = type as NonPrimitiveType;
  return t.type == TypeIdentifier.ReferenceType;
}
export function isNullableRefType(type: DataType): boolean {
  if (typeof type === "string") return false;
  const t: NonPrimitiveType = type as NonPrimitiveType;
  return t.type == TypeIdentifier.NullableReferenceType;
}

export function isNullable(type: DataType): boolean {
...
export function getLanguageSpecificName(
  nameMap: NameMap,
